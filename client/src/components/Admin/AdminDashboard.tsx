...
function getApiCVUrl(url?:string) {
 if(!url) return '';
 return url.replace(/^https?:\/\/localhost:5000\/uploads\//, 'api/uploads/');
}
                  ...
                  filteredUsers.map((user) => (
                    <tr key={user._id} className="hover:bg-gray-750 transition">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 py-1 text-xs rounded-full ${ user.type === 'candidate' ? 'bg-blue-500/20 text-blue-400' : 'bg-green-500/20 text-green-400'
                        }`}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 py-1 text-xs rounded-full ${
                          user.status === 'active' ? 'bg-green-500/20 text-green-400' : user.status === 'blocked' ? 'bg-red-500/20 text-red-400' : 'bg-yellow-500/20 text-yellow-400'
                        }`}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        {user.type === 'candidate' && user.CV ? (
                          <button
				 onClick={() => {
				 const apiCVUrl = getApiCVUrl(user.CV);   
				setViewingCV({ url: apiCVUrl, name: user.name });}}
          ...
          <div className="relative bg-gray-800 rounded-lg border-2 border-purple-400 p-6 max-w-2xl w-full">
            ...
            <div className="flex items-start gap-4 mb-6">
              {(selectedUser.photo || selectedUser.logo) ? (
                <img
                  src={selectedUser.photo || selectedUser.logo}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              ...
              <div>
                ...
                <p className="text-white capitalize">
                  {selectedUser.status === 'active' ? 'Actif' : selectedUser.status === 'blocked' ? 'Bloqué' : 'En attente'}
                  {selectedUser.type === 'company' && selectedUser.verified && ' (Vérifiée)'}
                </p>
              </div>
              <div>
...
{selectedUser.type === 'candidate' && selectedUser.adminCV && (
  <div>
    ...
    <button
      onClick={() => setViewingCV({ url: getApiCVUrl(selectedUser.adminCV!), name: selectedUser.name })}
    ...
    <a
      href={getApiCVUrl(selectedUser.adminCV)}
      target="_blank"
    ...
    <span className="text-xs text-gray-400 ml-2">
      ({getApiCVUrl(selectedUser.adminCV).split('/').pop()})
    </span>
  </div>
)}          </div>
    ...
    <div className="bg-gray-700 rounded-lg p-4 flex flex-col gap-3 shadow-inner">
      ...
      {selectedUser.adminCV ? (
        <div className="flex items-center gap-2 mb-2">
          ...
          >
            Voir le CV anonyme déjà ajouté
          </a>
          <span className="text-xs text-gray-400">
            ({selectedUser.adminCV.split('/').pop()})
          </span>
        </div>
      ) : (
